...
In de code kan \verb+log\_metrics+ en \verb+log\_model+ te zien zijn, deze zorgen ervoor dat bepaalde metrics te zien zijn bij de resultaten alsook dat het model word bijgehouden.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Kubeflow}
...
\end{minted}
In Kubeflow Pipelines is elke stap vertegenwoordigd door een \verb+pipeline\_op+ object in de pipeline. De invoer- en uitvoerparameters van elke \verb+pipeline\_op+ worden gespecificeerd om de gegevensstroom tussen de stappen te regelen. Bij het uitvoeren van de pipeline worden de functies in de opgegeven volgorde uitgevoerd, waarbij de uitvoer van de ene stap de invoer vormt voor de volgende stap. Dit zorgt voor een gestroomlijnde en reproduceerbare data pipeline voor het trainen en evalueren van machine learning modellen.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{ZenML}
...
\begin{itemize}
    ...
    \item \textbf{@pipeline:} Deze decorator gaat alle functies met de step decorator uitvoeren, deze decorator kan ook meerdere opties bevatten zoals het gebruiken van cache. Deze cache zorgt ervoor dat een al uitgevoerde step niet opnieuw wordt uitgevoerd als er geen verandering is.
\end{itemize}

Deze code heeft 2 steps die dan samen in de pipeline worden uitgevoerd. Eerst wordt de \verb+read\_data()+ uitgevoerd die dan zijn resultaten naar de functie train stuurt om het model te trainen.
...
\end{itemize}
Ondanks alle voordelen biedt Dagster ook enkele nadelen:
\begin{itemize}
...
\begin{minted}[frame=lines,breaklines, linenos]{python}
    ...
    def train():
        ...
        accuracy = accuracy_score(y_test, preds)
\end{minted}
In Dagster worden gegevens en taken gemodelleerd als \verb+assets+, die worden aangegeven met de @asset decorator. Deze decorator wordt gebruikt om aan te geven dat een functie een gegevensbron of een taak is binnen de data pipeline. Hierna kan van de webinterface elke asset uitgevoerd worden.
\subsection{Luigi}
